...
class Notebook:
    ...
    def add_note(self):
        ...
        title = input("Masukkan title catatan: ")
        # Periksa apakah title sudah ada untuk menghindari duplikasi
        if self.notes_list.find_node_by_title(title):
            print("Catatan dengan title tersebut sudah ada. Gunakan title lain.")
            return
        ...
        new_note = Note(title, full_content) # Buat objek Note baru
        self.notes_list.appends(new_note)    # Tambahkan Note ke linked list
        print(f"Catatan '{title}' berhasil ditambahkan ke notebook '{self.name}'.")
    ...
    def edit_note(self):
        # Mengedit title atau konten dari catatan yang sudah ada.
        title_to_edit = input("Masukkan title catatan yang ingin diedit: ")
        node_to_edit = self.notes_list.find_node_by_title(title_to_edit)
        ...
        if node_to_edit:
            ...
            if choice == '1':
                ...
                # Periksa apakah title baru sudah digunakan oleh catatan lain
                ada_new_title = self.notes_list.find_node_by_title(new_title)
                if ada_new_title and ada_new_title != node_to_edit:
                    print("title baru sudah digunakan oleh catatan lain. Gagal mengedit.")
                else:                    
                    node_to_edit.data.update_title(new_title)
                    print("title catatan berhasil diupdate.")
            elif choice == '2':
                # Edit konten catatan
            ...
            print(f"Catatan '{title_to_edit}' tidak ditemukan.")

    def delete_note(self):
        # Menghapus catatan dari buku catatan berdasarkan title.
        title_to_delete = input("Masukkan title catatan yang ingin dihapus: ")
        self.notes_list.delete_node_by_title(title_to_delete) # Panggil metode delete dari SinglyLinkedList

    def sort_notes(self):
        # Mengurutkan catatan dalam buku catatan berdasarkan title (A-Z).
    ...
    def search_note(self):
        # Mencari catatan dalam buku catatan berdasarkan title.
        title_to_search = input("Masukkan title catatan yang ingin dicari: ")
        self.notes_list.linear_search_by_title(title_to_search) # Panggil metode search dari SinglyLinkedList
